# ChusteRM Development Guidelines
...
- ChusteRM has an Enterprise overlay under `enterprise/` that extends/overrides OSS code.
...
- For user-facing strings that currently contain "ChusteRM" but should adapt to branded/self-hosted installs, prefer applying `replaceInstallationName` from `shared/composables/useBranding` in the UI layer (for example tooltip and suggestion labels) instead of adding hardcoded brand-specific copy.
