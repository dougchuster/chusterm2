...
describe PortalHelper do
  ...
  describe '#render_category_content' do
    ...
    let(:renderer) { instance_double(ChusteRMMarkdownRenderer) }

    before do
      allow(ChusteRMMarkdownRenderer).to receive(:new).with(markdown_content).and_return(renderer)
  ...
  describe '#set_og_image_url' do
    let(:portal_name) { 'ChusteRM Portal' }
    let(:title) { 'Welcome to ChusteRM' }
  ...
  describe '#generate_portal_brand_url' do
    it 'builds URL with UTM params and referer host as source (happy path)' do
      result = helper.generate_portal_brand_url('https://brand.example.com', 'https://app.chusterm.com/some/page')
      uri = URI.parse(result)
      params = Rack::Utils.parse_query(uri.query)
      ...
      expect(params['utm_source']).to eq('app.chusterm.com')
    end

    it 'returns utm string when brand_url is nil or empty' do
      expect(helper.generate_portal_brand_url(nil,
                                              'https://app.chusterm.com')).to eq(
                                                '?utm_campaign=branding&utm_medium=helpcenter&utm_source=app.chusterm.com'
                                              )
