...
RSpec.describe 'Contacts API', type: :request do
  ...
  describe 'GET /api/v1/accounts/{account.id}/contacts' do
    ...
    context 'when it is an authenticated user' do
      ...
      it 'keeps customer contacts visible in CRM v2 lists and customer filters' do
        account.enable_features!('crm_v2')
        customer_contact = create(:contact, :with_email, account: account, name: 'Cliente Visivel')
        customer_contact.update!(contact_type: 'customer', relationship_status: 'customer', lifecycle_stage: 'customer')
        visitor_contact = create(:contact, account: account, name: 'Visitante', email: nil, phone_number: nil, identifier: nil)
        ...
        expect(contact_ids).to include(contact.id, customer_contact.id)
        expect(contact_ids).not_to include(visitor_contact.id)

        get "/api/v1/accounts/#{account.id}/contacts?include_contact_inboxes=false&relationship_status=customer",
            headers: admin.create_new_auth_token,
            as: :json

        expect(response).to have_http_status(:success)
        customer_ids = response.parsed_body['payload'].pluck('id')
        expect(customer_ids).to include(customer_contact.id)
        expect(customer_ids).not_to include(contact.id)
      end
  ...
  describe 'PATCH /api/v1/accounts/{account.id}/contacts/:id' do
    ...
    context 'when it is an authenticated user' do
      ...
      it 'persists the CRM relationship when a lead becomes a customer' do
        contact.update!(contact_type: 'lead', relationship_status: 'lead', lifecycle_stage: 'lead')

        patch "/api/v1/accounts/#{account.id}/contacts/#{contact.id}",
              params: { relationship_status: 'customer', lifecycle_stage: 'customer' },
              headers: admin.create_new_auth_token,
              as: :json
        ...
        expect(json_response['relationship_status']).to eq('customer')
        expect(json_response['lifecycle_stage']).to eq('customer')

        contact.reload
        expect(contact).to be_customer
        expect(contact.relationship_status).to eq('customer')
        expect(contact.lifecycle_stage).to eq('customer')
        expect(contact.became_customer_at).to be_present
      end

      it 'persists the CRM relationship when a customer becomes a lead again' do
        contact.update!(contact_type: 'customer', relationship_status: 'customer', lifecycle_stage: 'customer')

        patch "/api/v1/accounts/#{account.id}/contacts/#{contact.id}",
              params: { relationship_status: 'lead', lifecycle_stage: 'lead' },
              headers: admin.create_new_auth_token,
              as: :json
        ...
        json_response = response.parsed_body['payload']
        expect(json_response['relationship_status']).to eq('lead')
        expect(json_response['lifecycle_stage']).to eq('lead')

        contact.reload
        expect(contact).to be_lead
        expect(contact.relationship_status).to eq('lead')
        expect(contact.lifecycle_stage).to eq('lead')
        expect(contact.became_lead_at).to be_present
      end
    end
  end
