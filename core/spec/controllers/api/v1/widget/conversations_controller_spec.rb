...
RSpec.describe '/api/v1/widget/conversations/toggle_typing', type: :request do
  ...
  def conversation_params
    {
      ...
      contact: {
        name: 'contact-name',
        email: 'contact-email@chusterm.com',
  ...
  describe 'POST /api/v1/widget/conversations' do
    it 'creates a conversation with correct details' do
      ...
      expect(json_response['contact']['email']).to eq 'contact-email@chusterm.com'
    ...
    it 'does not update the name if the contact already exist' do
      existing_contact = create(:contact, account: account, email: 'contact-email@chusterm.com')
  ...
  describe 'POST /api/v1/widget/conversations/set_custom_attributes' do
    let(:params) { { website_token: web_widget.website_token, custom_attributes: { 'product_name': 'ChusteRM' } } }
    ...
    context 'with correct website token' do
      it 'sets the values when provided' do
        ...
        expect(response).to have_http_status(:success)
        conversation.reload
        # conversation custom attributes should have "product_name" key with value "ChusteRM"
        expect(conversation.custom_attributes).to include('product_name' => 'ChusteRM')
      end
    end
  end
  ...
  describe 'POST /api/v1/widget/conversations/destroy_custom_attributes' do
    ...
    context 'with correct website token' do
      it 'sets the values when provided' do
        ...
        conversation.custom_attributes = { 'product_name': 'ChusteRM' }
        conversation.save!
        expect(conversation.custom_attributes).to include('product_name' => 'ChusteRM')
        ...
        expect(response).to have_http_status(:success)
        conversation.reload
        # conversation custom attributes should not have "product_name" key with value "ChusteRM"
        expect(conversation.custom_attributes).not_to include('product_name' => 'ChusteRM')
      end
    end
  end
end
