...
describe Email::SendOnEmailService do
  ...
  describe '#perform' do
    ...
    context 'when an error occurs' do
      ...
      let(:exception_tracker) { instance_double(ChusteRMExceptionTracker, capture_exception: true) }
      ...
      before do
        ...
        allow(ChusteRMExceptionTracker).to receive(:new).and_return(exception_tracker)
      end

      it 'captures the exception' do
        expect(ChusteRMExceptionTracker).to receive(:new).with(error, account: message.account)
