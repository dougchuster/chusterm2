...
RSpec.describe User do
  ...
  describe 'hmac_identifier' do
    it 'return nil if ChusteRM_INBOX_HMAC_KEY is not set' do
      expect(user.hmac_identifier).to eq('')
    end

    it 'return value if ChusteRM_INBOX_HMAC_KEY is set' do
      ConfigLoader.new.process
      i = InstallationConfig.find_by(name: 'ChusteRM_INBOX_HMAC_KEY')
  ...
  describe '2FA/MFA functionality' do
    before do
      skip('Skipping since MFA is not configured in this environment') unless ChusteRM.encryption_configured?
    end
    ...
    describe '#two_factor_provisioning_uri' do
      ...
      it 'generates a valid provisioning URI for QR code' do
        ...
        expect(uri).to include('ChusteRM')
      end
    end
