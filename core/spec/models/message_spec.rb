...
RSpec.describe Message do
    ...
    context 'when message is input_csat' do
      ...
      context 'when inbox is not web widget' do
        ...
        it 'returns only the base content without URL when survey_url stored separately' do
          message.content_attributes = { 'survey_url' => 'https://app.chusterm.com/survey/responses/12345' }
          expect(message.content).to eq('Rate your experience')
        end
      end
    end
  ...
  describe '#should_index?' do
    ...
    before do
      allow(ChusteRMApp).to receive(:advanced_search_allowed?).and_return(true)
      account.enable_features('advanced_search_indexing')
    end

    context 'when advanced search is not allowed globally' do
      before do
        allow(ChusteRMApp).to receive(:advanced_search_allowed?).and_return(false)
      end
    ...
    context 'when advanced search feature is not enabled for account on ChusteRM cloud' do
      before do
