...
describe Webhooks::Trigger do
  ...
  describe 'request headers' do
    ...
    context 'with delivery_id' do
      it 'adds X-ChusteRM-Delivery header' do
        expect(RestClient::Request).to receive(:execute) do |args|
          expect(args[:headers]['X-ChusteRM-Delivery']).to eq('test-uuid')
          expect(args[:headers]).not_to have_key('X-ChusteRM-Signature')
          expect(args[:headers]).not_to have_key('X-ChusteRM-Timestamp')
        end
    ...
    context 'with secret' do
      ...
      it 'adds X-ChusteRM-Timestamp header' do
        expect(RestClient::Request).to receive(:execute) do |args|
          expect(args[:headers]['X-ChusteRM-Timestamp']).to match(/\A\d+\z/)
        end
        trigger.execute(url, payload, webhook_type, secret: secret)
      end

      it 'adds X-ChusteRM-Signature header with correct HMAC' do
        expect(RestClient::Request).to receive(:execute) do |args|
          ts = args[:headers]['X-ChusteRM-Timestamp']
          expected_sig = "sha256=#{OpenSSL::HMAC.hexdigest('SHA256', secret, "#{ts}.#{body}")}"
          expect(args[:headers]['X-ChusteRM-Signature']).to eq(expected_sig)
        end
        trigger.execute(url, payload, webhook_type, secret: secret)
      end
      ...
      it 'signs timestamp.body not just body' do
        expect(RestClient::Request).to receive(:execute) do |args|
          args[:headers]['X-ChusteRM-Timestamp']
          wrong_sig = "sha256=#{OpenSSL::HMAC.hexdigest('SHA256', secret, body)}"
          expect(args[:headers]['X-ChusteRM-Signature']).not_to eq(wrong_sig)
        end
        trigger.execute(url, payload, webhook_type, secret: secret)
      end
    end
    ...
    context 'with both secret and delivery_id' do
      it 'includes all three security headers' do
        expect(RestClient::Request).to receive(:execute) do |args|
          expect(args[:headers]['X-ChusteRM-Delivery']).to eq('abc-123')
          expect(args[:headers]['X-ChusteRM-Timestamp']).to be_present
          expect(args[:headers]['X-ChusteRM-Signature']).to start_with('sha256=')
        end
