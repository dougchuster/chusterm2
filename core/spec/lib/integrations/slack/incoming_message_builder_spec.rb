  ...
  describe '#perform' do
    ...
    context 'when message creation' do
      ...
      it 'does not create message if message already exists' do
        ...
        expect(conversation.messages.last.content).to eql('this is test https://chusterm.com Hey @Sojan Test again')
      end
    ...
    context 'when resolving slack sender' do
      ...
      context 'when slack user email matches a ChusteRM agent' do
        before do
      ...
      context 'when slack user email does not match any ChusteRM agent' do
        before do
          slack_response = {
            user: {
      ...
      let(:link_shared) do
        {
          team_id: 'TLST3048H',
          api_app_id: 'A012S5UETV4',
          event: link_shared_event.merge({ links: [{ url: "https://qa.chusterm.com/app/accounts/1/conversations/#{conversation.display_id}",
