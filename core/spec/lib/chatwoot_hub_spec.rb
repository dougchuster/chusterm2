...
describe ChusteRMHub do
  describe '.base_url' do
    it 'uses the static hub url' do
      expect(described_class::DEFAULT_BASE_URL).to eq('https://hub.2.chusterm.com')
      expect(described_class.base_url).to eq('https://hub.2.chusterm.com')
    end
  end
  ...
  context 'when fetching sync_with_hub' do
    it 'get latest version from ChusteRM hub' do
      version = '1.1.1'
    ...
    it 'returns nil when ChusteRM hub is down' do
