    ...
    context 'with metadata headers' do
      ...
      it 'includes metadata headers in GET request' do
        stub_request(:get, 'https://example.com/api/data')
          .with(headers: {
                  'X-ChusteRM-Account-Id' => account.id.to_s,
                  'X-ChusteRM-Assistant-Id' => assistant.id.to_s,
                  'X-ChusteRM-Tool-Slug' => custom_tool.slug,
                  'X-ChusteRM-Conversation-Id' => conversation.id.to_s,
                  'X-ChusteRM-Conversation-Display-Id' => conversation.display_id.to_s,
                  'X-ChusteRM-Contact-Inbox-Id' => conversation.contact_inbox.id.to_s,
                  'X-ChusteRM-Contact-Inbox-Verified' => conversation.contact_inbox.hmac_verified.to_s,
                  'X-ChusteRM-Contact-Id' => contact.id.to_s,
                  'X-ChusteRM-Contact-Email' => contact.email
                })
      ...
      it 'defaults contact inbox verified header to false when contact inbox is missing' do
        ...
        stub_request(:get, 'https://example.com/api/data')
          .with(headers: {
                  'X-ChusteRM-Contact-Inbox-Verified' => 'false'
                })
          .to_return(status: 200, body: '{"success": true}')
        ...
        expect(WebMock).to have_requested(:get, 'https://example.com/api/data')
          .with(headers: { 'X-ChusteRM-Contact-Inbox-Verified' => 'false' })
      end
      ...
      it 'includes contact phone when present' do
        ...
        stub_request(:get, 'https://example.com/api/data')
          .with(headers: {
                  'X-ChusteRM-Contact-Phone' => '+1234567890'
                })
          .to_return(status: 200, body: '{"success": true}')
        ...
        expect(WebMock).to have_requested(:get, 'https://example.com/api/data')
          .with(headers: { 'X-ChusteRM-Contact-Phone' => '+1234567890' })
      end
