...
RSpec.describe Internal::CheckNewVersionsJob do
  ...
  it 'updates the plan info' do
    data = { 'version' => '1.2.3', 'plan' => 'enterprise', 'plan_quantity' => 1, 'ChusteRM_support_website_token' => '123',
             'ChusteRM_support_identifier_hash' => '123', 'ChusteRM_support_script_url' => '123' }
    allow(ChusteRMHub).to receive(:sync_with_hub).and_return(data)
    job
    ...
    expect(InstallationConfig.find_by(name: 'ChusteRM_SUPPORT_WEBSITE_TOKEN').value).to eq '123'
    expect(InstallationConfig.find_by(name: 'ChusteRM_SUPPORT_IDENTIFIER_HASH').value).to eq '123'
    expect(InstallationConfig.find_by(name: 'ChusteRM_SUPPORT_SCRIPT_URL').value).to eq '123'
  end
