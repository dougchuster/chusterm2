module Concerns::Toolable
  ...
  def add_base_headers(headers, state)
    headers['X-ChusteRM-Account-Id'] = state[:account_id].to_s if state[:account_id]
    headers['X-ChusteRM-Assistant-Id'] = state[:assistant_id].to_s if state[:assistant_id]
    headers['X-ChusteRM-Tool-Slug'] = slug if slug.present?
  end

  def add_conversation_headers(headers, conversation)
    headers['X-ChusteRM-Conversation-Id'] = conversation[:id].to_s if conversation[:id]
    headers['X-ChusteRM-Conversation-Display-Id'] = conversation[:display_id].to_s if conversation[:display_id]
  end

  def add_contact_headers(headers, contact)
    headers['X-ChusteRM-Contact-Id'] = contact[:id].to_s if contact[:id]
    headers['X-ChusteRM-Contact-Email'] = contact[:email].to_s if contact[:email].present?
    headers['X-ChusteRM-Contact-Phone'] = contact[:phone_number].to_s if contact[:phone_number].present?
  end

  def add_contact_inbox_headers(headers, contact_inbox)
    headers['X-ChusteRM-Contact-Inbox-Id'] = contact_inbox[:id].to_s if contact_inbox&.[](:id)
    headers['X-ChusteRM-Contact-Inbox-Verified'] = (contact_inbox&.[](:hmac_verified) || false).to_s
  end
