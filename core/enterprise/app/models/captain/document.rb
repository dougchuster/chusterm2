...
class Captain::Document < ApplicationRecord
  ...
  has_many :versions, class_name: 'Captain::DocumentVersion', dependent: :destroy
  belongs_to :account
  ...
  after_commit :create_version_snapshot, if: :should_version_document?
  ...
  def should_version_document?
    return false if destroyed?
    return false unless available?

    content.present? && (previously_new_record? || saved_change_to_content? || saved_change_to_status?)
  end

  def create_version_snapshot
    Captain::DocumentVersion.create_snapshot!(self)
  rescue StandardError => e
    Rails.logger.warn("[Captain KB] version snapshot failed for document #{id}: #{e.message}")
  end
