...
class Captain::Assistant::AgentRunnerService
  ...
  def process_agent_result(result)
    ...
    if result.respond_to?(:error) && result.error.present?
      Rails.logger.warn "[Captain V2] Agent returned error: #{result.error.message}"
      return recoverable_error_response(result.error.message)
    end

    output = result.output
    response = output.is_a?(Hash) ? output.with_indifferent_access : { 'response' => output.to_s, 'reasoning' => 'Processed by agent' }
    return recoverable_error_response('Blank agent response') if response['response'].blank?

    response['agent_name'] = result.context&.dig(:current_agent)
    response = apply_deterministic_crm_actions(response) if openai_compatible_gemini_model?
    response
  end

  def error_response(error_message)
    recoverable_error_response(error_message)
  end
  ...
  def apply_deterministic_crm_actions(response)
    Captain::Assistant::DeterministicCrmActionsService.new(
      assistant: @assistant,
      conversation: @conversation,
      response: response
    ).perform
  ...
  def build_and_wire_agents
    ...
    return [assistant_agent] if openai_compatible_gemini_model?
  ...
  def openai_compatible_gemini_model?
    ...
    Llm::Config.openai_compatible_gemini_model?(model)
  rescue StandardError => e
    Rails.logger.warn "[Captain V2] Could not detect Gemini OpenAI-compatible model: #{e.message}"
    false
  end
