  ...
  class << self
    ...
    def contact_basic_lines(contact)
      [
        ...
        (["- CRM relationship: #{sanitize_attr(contact[:relationship_status])}"] if contact[:relationship_status].present?),
        (["- CRM lifecycle stage: #{sanitize_attr(contact[:lifecycle_stage])}"] if contact[:lifecycle_stage].present?),
        (["- CRM owner: #{sanitize_attr(contact[:crm_owner_name])}"] if contact[:crm_owner_name].present?)
      ].flatten.compact
    end
    ...
    def build_deal_context(deal_context)
      return '' unless deal_context.is_a?(Hash) && deal_context.any?

      lines = []
      lines << "- Titulo: #{deal_context[:title]}" if deal_context[:title]
      lines << "- Etapa: #{deal_context[:stage_name]}" if deal_context[:stage_name]
      lines << "- Score: #{deal_context[:score]}/100" if deal_context[:score]
      lines << "- Area juridica: #{deal_context[:legal_area]}" if deal_context[:legal_area]
      lines << "- Tipo de caso: #{deal_context[:case_type]}" if deal_context[:case_type]
      lines << "- Urgencia: #{deal_context[:urgency_level]}" if deal_context[:urgency_level]
      lines << "- Documentos: #{deal_context[:documents_status]}" if deal_context[:documents_status]
      lines << "- Conflito: #{deal_context[:conflict_check_status]}" if deal_context[:conflict_check_status]
      lines << "- Resumo: #{deal_context[:summary]}" if deal_context[:summary]
      lines << "- Proxima acao recomendada: #{deal_context[:next_best_action]}" if deal_context[:next_best_action]
      return '' if lines.empty?

      "[Deal Context]\n#{lines.join("\n")}\nUse these details to contextualize your responses to the client.\n\n"
    end
  end
end
