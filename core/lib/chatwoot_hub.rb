...
class ChusteRMHub
  DEFAULT_BASE_URL = 'https://hub.2.chusterm.com'.freeze
  ...
  def self.pricing_plan
    ChusteRMApp.enterprise? ? 'enterprise' : 'community'
  end

  def self.pricing_plan_quantity
    ChusteRMApp.enterprise? ? ChusteRMApp.max_limit : 0
  end

  def self.support_config
    {
      support_website_token: InstallationConfig.find_by(name: 'ChusteRM_SUPPORT_WEBSITE_TOKEN')&.value,
      support_script_url: InstallationConfig.find_by(name: 'ChusteRM_SUPPORT_SCRIPT_URL')&.value,
      support_identifier_hash: InstallationConfig.find_by(name: 'ChusteRM_SUPPORT_IDENTIFIER_HASH')&.value
    }
  end
  ...
  def self.instance_config
    {
      ...
      installation_version: ChusteRM.config[:version],
...
ChatwootHub = ChusteRMHub unless defined?(ChatwootHub)

enterprise_hub = Rails.root.join('enterprise/lib/enterprise/chatwoot_hub.rb')
require enterprise_hub.to_s if enterprise_hub.exist?

ChusteRMHub.singleton_class.prepend_mod_with('ChusteRMHub')
