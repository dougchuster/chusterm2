...
class Account < ApplicationRecord
  ...
  store_accessor :custom_attributes, :onboarding_step
  ...
  has_many :crm_external_connections, dependent: :destroy_async
  ...
  has_many :captain_conversation_states, dependent: :destroy
  has_many :captain_playbooks, dependent: :destroy_async, class_name: 'Captain::Playbook'
  has_many :captain_document_versions, dependent: :destroy_async, class_name: 'Captain::DocumentVersion'
  ...
  has_one :evolution_api_configuration, dependent: :destroy
  ...
  def usage_limits
    {
      agents: ChusteRMApp.max_limit.to_i,
      inboxes: ChusteRMApp.max_limit.to_i,
      captain: {
        documents: captain_usage_limit(:captain_documents, :captain_documents_usage),
        responses: captain_usage_limit(:captain_responses, :captain_responses_usage)
      }
    }
  end

  def increment_response_usage
    increment_custom_usage!(:captain_responses_usage)
  end
  ...
  def captain_usage_limit(limit_key, usage_key)
    total_count = limits&.dig(limit_key.to_s).presence || ChusteRMApp.max_limit.to_i
    consumed = custom_attributes&.dig(usage_key.to_s).to_i

    {
      total_count: total_count.to_i,
      current_available: (total_count.to_i - consumed).clamp(0, total_count.to_i),
      consumed: consumed
    }
  end

  def increment_custom_usage!(usage_key)
    self.custom_attributes ||= {}
    custom_attributes[usage_key.to_s] = custom_attributes[usage_key.to_s].to_i + 1
    save
  end
