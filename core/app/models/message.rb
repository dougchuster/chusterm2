...
class Message < ApplicationRecord
  searchkick callbacks: false if ChusteRMApp.advanced_search_allowed?
  ...
  after_update_commit :track_campaign_delivery_status, if: :saved_change_to_status?
  ...
  def should_index?
    return false unless ChusteRMApp.advanced_search_allowed?
    ...
    return false if ChusteRMApp.ChusteRM_cloud? && !account.feature_enabled?('advanced_search_indexing')
  ...
  # Includes attachment understanding metadata when available.
  def content_for_llm
    parts = []
    parts << content if content.present?
    parts.concat(attachment_content_for_llm)
  ...
  def attachment_image_content_for_llm(attachment)
    description = attachment.meta&.dig('image_description').presence
    ocr_text = attachment.meta&.dig('ocr_text').presence
    document_guess = attachment.meta&.dig('document_guess').presence
  ...
  def track_campaign_reply
    ...
    Campaigns::DeliveryTracker.new(conversation.campaign).record!(
      :replied,
      contact: sender.is_a?(Contact) ? sender : conversation.contact,
      conversation: conversation,
      message: self,
      provider: inbox&.channel_type
    )
  rescue StandardError => e
    Rails.logger.warn("[Campaign Tracking] reply event failed for message #{id}: #{e.message}")
  end

  def track_campaign_delivery_status
    return unless outgoing? || template?
    return if conversation.campaign.blank?

    Campaigns::ProviderEventTracker.track_message_status!(self)
  end

  def track_campaign_opt_out
    return unless incoming? && !private?
    return unless sender.is_a?(Contact)
    return unless campaign_opt_out_message?

    sender.campaign_opt_out!(
      source: Campaigns::ProviderEventTracker.provider_for(self),
      campaign: conversation.campaign
    )
  rescue StandardError => e
    Rails.logger.warn("[Campaign Tracking] opt-out failed for message #{id}: #{e.message}")
  end

  def campaign_opt_out_message?
    normalized = content.to_s.strip.downcase
    %w[sair stop cancelar descadastrar unsubscribe].include?(normalized)
  end
  ...
  def captain_pending_conversation?
    ...
    ::CaptainInbox.exists?(inbox_id: conversation.inbox_id, enabled: true)
  end
