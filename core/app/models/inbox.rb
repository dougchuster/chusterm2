...
class Inbox < ApplicationRecord
  ...
  def active_bot?
    agent_bot_inbox&.active? || captain_active? || hooks.where(app_id: %w[dialogflow],
                                                               status: 'enabled').count.positive?
  end

  def captain_active?
    captain_inbox&.active_for_auto_reply? && more_captain_responses?
  end
