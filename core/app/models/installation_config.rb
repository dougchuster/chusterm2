...
class InstallationConfig < ApplicationRecord
  ...
  before_validation :normalize_llm_config_values
  ...
  def normalize_llm_config_values
    return unless name.in?(%w[
      CAPTAIN_OPEN_AI_MODEL CAPTAIN_OPEN_AI_ENDPOINT
      CAPTAIN_MEDIA_AI_MODEL CAPTAIN_MEDIA_AI_ENDPOINT
      CAPTAIN_AUDIO_TRANSCRIPTION_MODEL CAPTAIN_AUDIO_TRANSCRIPTION_ENDPOINT
      CAPTAIN_EMBEDDING_MODEL CAPTAIN_EMBEDDING_ENDPOINT
    ])

    if name.in?(%w[CAPTAIN_OPEN_AI_MODEL CAPTAIN_MEDIA_AI_MODEL CAPTAIN_AUDIO_TRANSCRIPTION_MODEL CAPTAIN_EMBEDDING_MODEL])
      self.value = Llm::Config.normalize_model(value)
    elsif name.in?(%w[CAPTAIN_OPEN_AI_ENDPOINT CAPTAIN_MEDIA_AI_ENDPOINT CAPTAIN_AUDIO_TRANSCRIPTION_ENDPOINT CAPTAIN_EMBEDDING_ENDPOINT])
      self.value = Llm::Config.normalize_endpoint(value)
    end
  end
end
