...
export default {
  ...
  mounted() {
    const { websiteToken, locale, widgetColor } = window.ChusteRMWebChannel;
  ...
  methods: {
    ...
    setLocale(localeWithVariation) {
      ...
      const { enabledLanguages } = window.ChusteRMWebChannel;
    ...
    registerCampaignEvents() {
      ...
      emitter.on('execute-campaign', campaignDetails => {
        ...
        const { websiteToken } = window.ChusteRMWebChannel;
