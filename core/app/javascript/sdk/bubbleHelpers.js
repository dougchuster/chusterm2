...
export const setBubbleText = bubbleText => {
  if (isExpandedView(window.$ChusteRM.type)) {
...
export const createBubbleIcon = ({ className, path, target }) => {
  let bubbleClassName = `${className} woot-elements--${window.$ChusteRM.position}`;
...
const handleBubbleToggle = newIsOpen => {
  ...
  if (newIsOpen) {
    dispatchWindowEvent({ eventName: ChusteRM_OPENED });
  } else {
    dispatchWindowEvent({ eventName: ChusteRM_CLOSED });
    chatBubble.focus();
  }
};
...
export const onBubbleClick = (props = {}) => {
  ...
  const { isOpen } = window.$ChusteRM;
  if (isOpen === toggleValue) return;

  const newIsOpen = toggleValue === undefined ? !isOpen : toggleValue;
  window.$ChusteRM.isOpen = newIsOpen;
