...
const handleUpdateConfig = async payload => {
  try {
    await store.dispatch('captainInboxes/update', {
      assistantId: assistantId.value,
      ...payload,
    });
    useAlert(t('CAPTAIN.INBOXES.CONFIG.SUCCESS_MESSAGE'));
  } catch (error) {
    const errorMessage =
      error?.message || t('CAPTAIN.INBOXES.CONFIG.ERROR_MESSAGE');
    ...
    <template #body>
      <div class="flex flex-col gap-4">
        <InboxCard
          ...
          :key="captainInbox.id"
          :inbox="captainInbox"
          :is-updating="uiFlags.updatingItem"
          @action="handleAction"
