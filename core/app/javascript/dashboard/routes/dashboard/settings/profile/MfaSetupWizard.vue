...
const downloadBackupCodes = () => {
  const codesText = `ChusteRM Two-Factor Authentication Backup Codes\n\n${props.backupCodes.join('\n')}\n\nKeep these codes in a safe place.`;
  const blob = new Blob([codesText], { type: 'text/plain' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = 'ChusteRM-backup-codes.txt';
  a.click();
...
<template>
  <div v-if="showSetup && !mfaEnabled">
    ...
    <div v-if="setupStep === 'qr'" class="space-y-6">
      <div
        class="bg-n-solid-1 rounded-xl outline-1 outline-ui-border-subtle outline p-10 flex flex-col gap-4"
      >
        ...
        <div class="flex justify-center">
          <div
            class="bg-n-background p-4 rounded-lg outline outline-1 outline-ui-border-subtle"
          >
            <img
              v-if="qrCodeUrl"
              :src="qrCodeUrl"
    ...
    <div v-if="setupStep === 'backup'" class="space-y-6">
      ...
      <!-- Warning Alert -->
      <div
        class="flex items-start gap-2 p-4 bg-n-solid-1 outline outline-ui-border-subtle rounded-xl outline-1"
      >
        <Icon
      ...
      <!-- Backup Codes Grid -->
      <div
        class="bg-n-solid-1 rounded-xl outline-1 outline-ui-border-subtle outline flex flex-col gap-6 p-6"
      >
