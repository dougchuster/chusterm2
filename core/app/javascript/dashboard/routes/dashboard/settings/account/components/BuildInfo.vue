...
const latestChusteRMVersion = computed(() => {
  return currentAccount.value.latest_chusterm_version;
});

const globalConfig = useMapGetter('globalConfig/get');

const hasAnUpdateAvailable = computed(() => {
  if (!semver.valid(latestChusteRMVersion.value)) {
    return false;
  }

  return semver.lt(globalConfig.value.appVersion, latestChusteRMVersion.value);
});
...
<template>
  <div class="p-4 text-sm text-center">
    <div v-if="hasAnUpdateAvailable && globalConfig.displayManifest">
      {{
        t('GENERAL_SETTINGS.UPDATE_ChusteRM', {
          latestChusteRMVersion: latestChusteRMVersion,
        })
      }}
    </div>
