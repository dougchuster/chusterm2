...
const { isOnChusteRMCloud } = useAccount();
...
const fetchSSLStatus = () => {
  if (!isOnChusteRMCloud.value) return;
...
const handleUpdatePortalConfiguration = portalObj => {
  ...
  // If custom domain is added or updated, fetch SSL status after a delay of 5 seconds (only on ChusteRM cloud)
  if (portalObj?.custom_domain && isOnChusteRMCloud.value) {
    setTimeout(() => {
      fetchSSLStatus();
