...
export const actions = {
  search: async (
    { commit },
    { search, page, sortAttr, label, append = false, crmFilters = {} }
  ) => {
  ...
  get: async (
    { commit },
    { page = 1, sortAttr, label, crmFilters = {} } = {}
  ) => {
    commit(types.SET_CONTACT_UI_FLAG, { isFetching: true });
    try {
      const {
        data: { payload, meta },
      } = await ContactAPI.get(page, sortAttr, label, crmFilters);
      commit(types.CLEAR_CONTACTS);
  ...
  active: async ({ commit }, { page = 1, sortAttr, crmFilters = {} } = {}) => {
    commit(types.SET_CONTACT_UI_FLAG, { isFetching: true });
    try {
      const {
        data: { payload, meta },
      } = await ContactAPI.active(page, sortAttr, crmFilters);
      commit(types.CLEAR_CONTACTS);
      commit(types.SET_CONTACTS, payload);
      commit(types.SET_CONTACT_META, meta);
      commit(types.SET_CONTACT_UI_FLAG, { isFetching: false });
  ...
  import: async ({ commit }, { file, options = {} }) => {
    commit(types.SET_CONTACT_UI_FLAG, { isImporting: true });
    try {
      await ContactAPI.importContacts(file, options);
  ...
  filter: async (
    ...
    try {
      const {
        data: { payload, meta },
      } = await ContactAPI.filter(page, sortAttr, queryPayload, crmFilters);
      if (resetState) {
        commit(types.CLEAR_CONTACTS);
        commit(types.SET_CONTACTS, payload);
        commit(types.SET_CONTACT_META, meta);
