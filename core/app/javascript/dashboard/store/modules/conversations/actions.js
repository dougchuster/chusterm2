...
export const hasMessageFailedWithExternalError = pendingMessage => {
  ...
  // 2. Messages sent from ChusteRM but failed to deliver to the customer for some reason (user blocking or client system down):
...
const actions = {
  ...
  addConversation({ commit, state, dispatch, rootState }, conversation) {
    const { currentInbox, appliedFilters, chatStatusFilter } = state;
    const {
      inbox_id: inboxId,
      meta: { sender },
      status,
    ...
    const isMatchingStatusFilter = filterByStatus(
      status,
      chatStatusFilter,
      conversation
    );
