...
<template>
  <BaseBubble class="px-4 py-3" data-bubble-name="csat">
    ...
    <dl v-if="isRatingSubmitted" class="mt-4">
      <dt class="italic text-ds-fg-muted">
      ...
      <dd v-else class="flex mt-1">
        <span v-for="n in 5" :key="n" class="mr-1 text-2xl">
          <i
            :class="[
              n <= starRatingValue
                ? 'i-lucide-star fill-ds-state-warning text-ds-state-warning'
                : 'i-lucide-star text-ds-fg-subtle',
            ]"
          />
        </span>
      </dd>

      <dt v-if="response.feedbackMessage" class="mt-2 italic text-ds-fg-muted">
