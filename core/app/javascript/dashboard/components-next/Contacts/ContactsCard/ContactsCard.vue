...
const props = defineProps({
  ...
  crmOwnerId: { type: [Number, String], default: null },
  crmOwner: { type: Object, default: null },
...
const crmContact = computed(() => ({
  relationshipStatus: props.relationshipStatus,
  lifecycleStage: props.lifecycleStage,
  crmOwnerId: props.crmOwnerId,
  crmOwner: props.crmOwner,
}));

const sourceList = computed(
  () =>
    props.additionalAttributes?.sourceList ||
    props.additionalAttributes?.source_list
);

const legalArea = computed(
  () =>
    props.additionalAttributes?.legalArea ||
    props.additionalAttributes?.legal_area
);

const visibleLabels = computed(() => props.labels?.slice(0, 4) || []);

const hiddenLabelsCount = computed(() =>
  Math.max((props.labels?.length || 0) - visibleLabels.value.length, 0)
);
...
<template>
  <div class="relative">
    ...
    >
      <div
        class="grid w-full grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 sm:gap-4"
      >
        <div
          class="relative"
        ...
        <div class="min-w-0">
          ...
          <CRMContactSummary
            ...
            compact
            editable
            @update="handleCrmUpdate"
          />
          <div
            v-if="visibleLabels.length || sourceList || legalArea"
            class="mt-2 flex flex-wrap items-center gap-1.5 text-xs"
          >
            <span
              v-if="sourceList"
              class="inline-flex h-6 max-w-full items-center gap-1 rounded bg-n-blue-3 px-2 font-medium text-n-blue-11 ring-1 ring-n-blue-7"
            >
              <span class="i-lucide-list-tree size-3.5 shrink-0" />
              <span class="truncate">{{ sourceList }}</span>
            </span>
            <span
              v-if="legalArea"
          ...
          <div
            class="mt-1.5 flex flex-wrap items-center justify-start gap-x-3 gap-y-1"
          >
