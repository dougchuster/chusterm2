...
const inboxHasAutoCaptain = computed(() => {
  const captainInbox = inbox.value?.captain_inbox || {};
  return captainInbox.active_for_auto_reply || captainInbox.responsible;
});
const anyInboxHasAutoCaptain = computed(() =>
  inboxesList.value.some(item => {
    const captainInbox = item?.captain_inbox || {};
    return captainInbox.active_for_auto_reply || captainInbox.responsible;
  })
);
const shouldUseAllTabForCaptain = computed(() => {
  return (
    inboxHasAutoCaptain.value ||
    (!activeInbox.value && anyInboxHasAutoCaptain.value)
  );
});
...
onMounted(() => {
  ...
  if (shouldUseAllTabForCaptain.value) {
    activeAssigneeTab.value = wootConstants.ASSIGNEE_TYPE.ALL;
  }
...
watch(shouldUseAllTabForCaptain, isActive => {
  if (isActive && activeAssigneeTab.value === wootConstants.ASSIGNEE_TYPE.ME) {
    activeAssigneeTab.value = wootConstants.ASSIGNEE_TYPE.ALL;
    resetAndFetchData();
  }
});
...
<template>
  ...
  >
    ...
    <div
      ...
      class="chat-list-scroll flex-1 min-h-0 overflow-y-auto conversations-list"
