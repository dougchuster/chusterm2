...
/**
 * WhatsApp inbox phone for Evolution: E.164 with +, or international digits only (no +).
 * Does not require the user to type "+" — we normalize before save.
 * @param {string} value
 * @returns {boolean}
 */
export const isEvolutionInboxPhone = value => {
  if (value === '' || value == null) return false;
  const trimmed = String(value).trim().replace(/\s/g, '');
  if (!trimmed) return false;
  if (/^\+[1-9]\d{1,14}$/.test(trimmed)) return true;
  return /^[1-9]\d{6,14}$/.test(trimmed);
};

/**
 * Normalizes Evolution / webhook identifier to E.164 (+ and digits only).
 * @param {string} value
 * @returns {string}
 */
export const normalizeToE164Phone = value => {
