class Notification::PushNotificationService
  ...
  def perform
    ...
    notification_subscriptions.each do |subscription|
      ...
      send_push_via_ChusteRM_hub(subscription)
    end
  end
  ...
  def handle_browser_push_error(error, subscription)
    ...
    when Errno::ECONNRESET, Net::OpenTimeout, Net::ReadTimeout
      ...
      ChusteRMExceptionTracker.new(error, account: notification.account).capture_exception
      true
    end
  end
  ...
  def send_push_via_ChusteRM_hub(subscription)
    return if firebase_credentials_present?
    return unless ChusteRM_hub_enabled?
    return unless subscription.fcm?

    ChusteRMHub.send_push(fcm_options(subscription))
  end
