class SearchService
  ...
  TIME_FILTER_COLUMNS = {
    'conversations.last_activity_at' => Conversation.arel_table[:last_activity_at],
    'messages.created_at' => Message.arel_table[:created_at],
    'last_activity_at' => Contact.arel_table[:last_activity_at],
    'updated_at' => Article.arel_table[:updated_at]
  }.freeze
  ...
  def should_run_advanced_search?
    ChusteRMApp.advanced_search_allowed? && current_account.feature_enabled?('advanced_search')
  end
  ...
  def apply_time_filter(query, column_name)
    ...
    column = safe_time_filter_column(column_name)
  ...
  def safe_time_filter_column(column_name)
    TIME_FILTER_COLUMNS.fetch(column_name)
  end
