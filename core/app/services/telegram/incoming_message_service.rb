...
class Telegram::IncomingMessageService
  ...
  def perform
    # ChusteRM doesn't support group conversations at the moment
    ...
    # ChusteRM defines a 'read' status in its enum but does not currently update this status for Telegram conversations.
    # We have two options:
    # 1. Send the read request to Telegram here, immediately when the message is created.
    # 2. Properly update the read status in the ChusteRM UI and trigger the Telegram request when the agent actually reads the message.
