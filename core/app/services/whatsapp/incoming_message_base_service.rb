...
class Whatsapp::IncomingMessageBaseService
  ...
  def process_statuses
    ...
    unless find_message_by_source_id(status[:id])
      track_campaign_status_without_message(status)
      return
    end
  ...
  def track_campaign_status_without_message(status)
    Campaigns::ProviderEventTracker.new(provider: whatsapp_campaign_provider).track_status!(
      external_id: status[:id],
      status: status[:status],
      metadata: whatsapp_status_metadata(status)
    )
  end

  def whatsapp_campaign_provider
    inbox.channel.respond_to?(:provider) ? inbox.channel.provider : inbox.channel_type
  end

  def whatsapp_status_metadata(status)
    metadata = {}
    metadata[:recipient_id] = status[:recipient_id] if status[:recipient_id].present?
    metadata[:timestamp] = status[:timestamp] if status[:timestamp].present?
    metadata[:errors] = status[:errors] if status[:errors].present?
    metadata
  end
