class ConversationFinder
  ...
  def filter_by_status
    return if (params[:status].presence || DEFAULT_STATUS) == 'all'

    @conversations = @conversations.where(status: statuses_for_request)
  end

  def statuses_for_request
    requested_status = params[:status].presence || DEFAULT_STATUS
    return Conversation.statuses.values if requested_status == 'all'
    return [requested_status] unless requested_status == 'open'
    return [requested_status] unless selected_inboxes_with_active_captain?

    [Conversation.statuses[:open], Conversation.statuses[:pending]]
  end

  def selected_inboxes_with_active_captain?
    Inbox.where(id: @inbox_ids).includes(:captain_inbox).any?(&:captain_responsible?)
  end
