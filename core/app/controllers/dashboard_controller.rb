class DashboardController < ActionController::Base
  ...
  def set_global_config
    ...
    @global_config['INSTALLATION_PRICING_PLAN'] = 'enterprise' if ChusteRMApp.enterprise?
  end
  ...
  def ensure_installation_onboarding
    key = defined?(::Redis::Alfred::ChusteRM_INSTALLATION_ONBOARDING) ?
            ::Redis::Alfred::ChusteRM_INSTALLATION_ONBOARDING :
            ::Redis::Alfred::CHATWOOT_INSTALLATION_ONBOARDING
    redirect_to '/installation/onboarding' if ::Redis::Alfred.get(key)
  end
  ...
  def app_config
    {
      APP_VERSION: (defined?(ChusteRM) ? ChusteRM : Chatwoot).config[:version],
  ...
  def allowed_login_methods
    ...
    methods << 'saml' if (defined?(ChusteRMHub) ? ChusteRMHub : ChatwootHub).pricing_plan != 'community' && GlobalConfigService.load('ENABLE_SAML_SSO_LOGIN', 'true').to_s != 'false'
    methods
  end
