class Api::V1::Accounts::AgentsController < Api::V1::Accounts::BaseController
  ...
  def update
    @agent.update!(agent_profile_params)
    @agent.current_account_user.update!(agent_account_user_params)
  end
  ...
  def new_agent_params
    permitted_params = params.require(:agent).permit(:email, :name, :auto_offline).to_h.with_indifferent_access
    assign_valid_account_user_enum!(permitted_params, :role, AccountUser.roles.keys)
    assign_valid_account_user_enum!(permitted_params, :availability, AccountUser.availabilities.keys)
    permitted_params
  end

  def agent_account_user_params
    permitted_params = params.require(:agent).permit(:auto_offline).to_h.with_indifferent_access
    assign_valid_account_user_enum!(permitted_params, :role, AccountUser.roles.keys)
    assign_valid_account_user_enum!(permitted_params, :availability, AccountUser.availabilities.keys)
    permitted_params.slice(*account_user_attributes).compact
  end

  def assign_valid_account_user_enum!(attributes, key, allowed_values)
    raw_value = params.require(:agent)[key]
    return if raw_value.blank?

    value = raw_value.to_s
    raise ActionController::BadRequest, "Invalid #{key}" unless allowed_values.include?(value)
