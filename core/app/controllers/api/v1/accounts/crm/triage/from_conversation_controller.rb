# Resolves conversation_id as the CRM foreign key (the database primary key).
class Api::V1::Accounts::Crm::Triage::FromConversationController < Api::V1::Accounts::Crm::BaseController
  def create
    ...
    render json: serialize_deal(deal, conversation), status: :created
  ...
  def serialize_deal(deal, conversation)
    {
      id: deal.id,
      title: deal.title,
      status: deal.status,
